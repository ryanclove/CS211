...
double ** invert (double ** matrix, int row, int column) {
	...
	/*go row by row, to make into an upper triangular matrix*/
		/*Make the first element of the row 1 by dividing by inverse[i][i]
	...
	/*turn the left matrix into an identity matrix*/
		/*subtract the row from all above it by a factor of matrix[j][i]*/
		/*then do the same to the inverse matrix*/
	for (i=row-1; i>0; i--) {
		for (j=i-1; j>=0; j--) {
			subtractor = matrix[j][i];
			subtractRow(matrix, i, j, subtractor, row);
...
int main (int argc, char** argv) {
	...
	/*Opens train file and puts data in x and y*/
	FILE * fp;
	fp = fopen(file1, "r");
	if (!fp) return 1;
	...
	/*no longer need the first inverse and the transposed X, free it*/
	...
	/*Open data file and initialize the data matrix*/
	FILE * fp2;
	fp2 = fopen(file2, "r");
	if (!fp2) return 1;
