 /* Takes in stdin or file readings as instructions for a truth table
	...
	Prints a Truth Table corresponding to the instructions given in the file or stdin
*/
...
void solveNodeA (struct node * ptr, char ** inputs, char ** outputs, int nIn, int nOut, int rowNum, short ** table, struct node * start) {
	...
	/*DO THE MATH AND CREATE THE OUTPUT ROW*/
...
void solveMuxNode (struct node * ptr, char ** inputs, char ** outputs, int nIn, int nOut, int rows, short ** table, struct node * start) {
	...
	/*checks if string output determiners are solved*/
	...
	/* math for the multiplier,  find entry to select based on the selectors, and then returns that input*/
	int entry=0;
	for (i=0; i<rows; i++) {
		entry = 0;
...
int main (int argc, char ** argv) {
	...
	/*read from stdin or file and the header*/
	...
	if (readFile==1) {
		...
		fp = stdin;
			...
			while (fscanf(fp, "%s ", read)==1) {
				/*SECTION 1-5 IF AND OR NAND NOR XOR are read*/
				if (strcmp(read, "AND")==0 || strcmp(read, "OR")==0 || 
					...
					free(out);
					free(B);
				/*SECTION IF NOT PASS6 or 7  are read*/
				...
				/*SECTION 8 - A MULTIPLEXER LINE IS READ*/
				...
				/*if type 9 (decoder)*/
					...
					/*create array of outputs as shorts*/
